...
var taille_table: Vector2 = Vector2(96, 64)
...
func _table_mobile(noeud: Control, texte: String, x: float, y: float) -> void:
	...
	table.global_position = Vector2(x, y) - taille_table / 2
...
func _on_ajouter_table_pressed() -> void:
	...
	if nombre >= 50:
		return 
	var x: float = %AjouterTable.global_position.x + %AjouterTable.size.x + taille_table.x + nombre * 4
	var y: float = %AjouterTable.global_position.y + taille_table.y + nombre * 6
...
func _on_retourner_plan_pressed() -> void:
	...
	var rectangle: Rect2 = _definir_rectangle()
	var centre: Vector2 = Vector2(rectangle.position.x + rectangle.size.x / 2, rectangle.position.y + rectangle.size.y / 2)
	for noeud in %PorteTables.get_children():
		noeud.position = 2 * centre - noeud.position - taille_table
	for noeud in %PorteTablesVierges.get_children():
		noeud.position = 2 * centre - noeud.position - taille_table	
	son_a_emettre.emit("valider")

func _definir_rectangle() -> Rect2:
	...
	return Rect2(Vector2(posx, posy), Vector2(largeur, longueur))
...
func _on_capture_pressed() -> void:
	...
	tout_l_ecran.resize(get_viewport_rect().size.x, get_viewport_rect().size.y)
